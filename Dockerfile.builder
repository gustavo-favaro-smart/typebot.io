...
FROM base AS pruned
COPY . .
RUN bunx turbo@2.4.5-canary.7 prune "builder" --docker
...
RUN SKIP_ENV_CHECK=true bunx turbo build --filter="builder"

# ================== RELEASE ======================

FROM base AS release
COPY --from=builder /app/node_modules ./node_modules
COPY --from=builder /app/packages/prisma/mysql ./packages/prisma/postgresql
...
ENTRYPOINT ["sh", "builder-entrypoint.sh"]
